 ...
 *
 * - customers
 * - customer_transaction_links
 * - customer_transaction_sync_state
 * - customer_events
 *
 * This service is deliberately independent from authorization/capture and is
 * not wired into the certified payment request path. It can be executed by a
 * background synchronizer without changing provider, ledger or wallet state.
 */
export const projectCustomerFromTransaction = async (
    ...
    const [customer] = await tx.$queryRaw<CustomerIdRow[]>(Prisma.sql`
      ...
      DO UPDATE SET
        ...
        normalized_phone = COALESCE(
          NULLIF(EXCLUDED.normalized_phone, ''),
  ...
  const [aggregate] = await prisma.$queryRaw<AggregateRow[]>(Prisma.sql`
    SELECT
      ...
      COALESCE(
        AVG(amount) FILTER (WHERE lower(status) = 'succeeded'),
        0
      ) AS avg_order,
      MIN(created_at) AS first_seen,
      MAX(created_at) AS last_seen,
      MAX(created_at) FILTER (WHERE lower(status) = 'succeeded') AS last_payment_at
    FROM transactions
