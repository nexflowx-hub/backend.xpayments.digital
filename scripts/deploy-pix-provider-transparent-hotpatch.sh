...
DIRECT_SHA="$(sha_in_container /app/dist/modules/payments/controllers/direct.controller.js)"
PIX_SHA="$(sha_in_container /app/dist/modules/payments/controllers/pix.controller.js)"
MISTIC_SHA="$(sha_in_container "$TARGET")"
WEBHOOK_SHA="$(sha_in_container /app/dist/modules/payments/controllers/misticpay-webhook.controller.js)"
ROUTES_SHA="$(sha_in_container /app/dist/modules/payments/routes/payments.routes.js)"
...
section "3. Patch only current compiled PIX provider gate"
...
docker start "$BUILD_CONTAINER" >/dev/null 2>&1 || true
CANDIDATE_TARGET_SHA="$(docker exec "$BUILD_CONTAINER" sha256sum "$TARGET" | awk '{print $1}')"
...
DIRECT_AFTER="$(sha_in_container /app/dist/modules/payments/controllers/direct.controller.js)"
PIX_AFTER="$(sha_in_container /app/dist/modules/payments/controllers/pix.controller.js)"
WEBHOOK_AFTER="$(sha_in_container /app/dist/modules/payments/controllers/misticpay-webhook.controller.js)"
ROUTES_AFTER="$(sha_in_container /app/dist/modules/payments/routes/payments.routes.js)"
...
echo "BASELINE_IMAGE=${BASELINE_IMAGE}"
echo "CANDIDATE_IMAGE=${CANDIDATE_IMAGE}"
echo "No payment, transaction, wallet or ledger operation was created by this deployment."
